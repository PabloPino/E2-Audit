...
public interface SponsorshipRepository extends JpaRepository<Sponsorship, Integer> {
	...
	@Query("select avg(1.0 * (select count(*) from Sponsorship s where s.position = p.id)) from Position p")
	Double queryRookiesA2AVG();

	@Query("select min(1.0 * (select count(*) from Sponsorship s where s.position = p.id)) from Position p")
	Double queryRookiesA2MIN();

	@Query("select max(1.0 * (select count(*) from Sponsorship s where s.position = p.id)) from Position p")
	Double queryRookiesA2MAX();

	@Query("select stddev(1.0 * (select count(*) from Sponsorship s where s.position = p.id)) from Position p")
	Double queryRookiesA2STDDEV();

	@Query("select p from Sponsorship s join s.provider p group by  p having count(s)>1.1*(select avg(1.0*(select count(t) from Sponsorship t where t.provider.id=x.id))from Provider x)")
